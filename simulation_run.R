...
calc_cumeff = function(tl_hist, fvar, flag){
  ...
  l_effect_lags = l_effect_seqs %>% map(~(NROW(.)-1):0)
...
sim_fit_and_res = function(nsub, t_max, tl_values, t_load_type, tl_var, fvar, flag, predvalues, i, folder, direction_flip = FALSE){
  ...
  d_res = bind_rows(d_ra, d_ewma, d_redi, d_dlnm)
    ...
    d_sim_tl_hist_acwr_acute = d_tl_hist %>% filter(day >= lag_max-5)
    d_sim_hist_acute = function_on_list(d_sim_tl_hist_acwr_acute, FUN = slide_sum, lag_max+1) %>% rename(acute_load = data)
    d_sim_hist_chronic = function_on_list(d_tl_hist, FUN = slide_chronic, lag_max+1) %>% rename(chronic_load = data)
    ...
    cp_preds_dlnm = crosspred(cb_dlnm_change, fit_dlnm_change, at = predvalues, cumul = TRUE)
